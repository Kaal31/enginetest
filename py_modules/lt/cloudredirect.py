...
# The actual save-redirect library headcrab's crinstall downloads (we were only
# doing the flatpak app before — this .so is what makes redirection work).
#
# Two sources, chosen by build variant:
#   * Selectively11 (upstream) — the DLC build (v2) uses this.
#   * cloudredirect-moon (swwayps) — a fork of Selectively11 matched to our moon
#     engine, adding cross-distro attach fixes, legacy save-layout healing, and
#     worker-thread crash containment on the 32-bit hook. The SIMPLE build (v1)
#     uses this. The fork ships no release/flatpakrepo, only the committed .so on
#     master (a real ELF, not LFS), so we swap ONLY the injected library here and
#     keep Selectively11's flatpak app below (same app id — it's a fork, so the
#     config/token layout is identical and the moon .so reads it fine).
CR_LIB_URL = "https://github.com/Selectively11/h3adcr-b/releases/download/linux-test/cloud_redirect.so"
CR_LIB_URL_MOON = "https://github.com/swwayps/cloudredirect-moon/raw/refs/heads/master/cloud_redirect.so"
...
def _cr_lib_url() -> str:
    """The CloudRedirect .so URL for this build: moon fork on the simple build,
    Selectively11 upstream on the DLC build."""
    return CR_LIB_URL_MOON if _is_simple_variant() else CR_LIB_URL
...
def _download_cr_lib() -> str:
    ...
    try:
        ...
        logger.log(f"CloudRedirect: fetching {src} cloud_redirect.so ({len(data)} bytes)")
        wrote = 0
        for d in _cr_dirs():
            try:
